...
int tcpUdpClass::init_epoll()
{
	...
	for ( std::map<int, gateway_handle_t*>::iterator i = m_lister_fd_map.begin();i != m_lister_fd_map.end(); ++i)
	{
	...
	for (std::map<int, gateway_udp_handle_t*>::iterator i = m_udp_fd_map.begin(); i != m_udp_fd_map.end(); ++i)
	{
		server_epoll_event.data.fd = i->first;
		if (epoll_ctl(m_epoll,EPOLL_CTL_ADD,i->first,&server_epoll_event) == -1){
			i->second->error(-1,"add epoll fail");
			m_udp_fd_map.erase(i->first);
			close(i->first);
		}
		else{
			++m_online;
...
void tcpUdpClass::deal_udp_fd(int fd)
{
	int  len;
...
void tcpUdpClass::deal_client_fd(int fd)
{
